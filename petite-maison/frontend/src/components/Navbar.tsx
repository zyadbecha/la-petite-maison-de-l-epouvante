...
import { ShoppingCart, User, BookOpen, Menu, X, Skull, Package } from "lucide-react";
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        await api.syncUser(token, { email: user?.email, display_name: user?.name, avatar_url: user?.picture });
          ...
          {isAuthenticated ? (
            <>
              ...
              <button
                ...
                className="text-xs text-bone/50 hover:text-blood transition-colors ml-2"
              >
                Déconnexion
              </button>
            </>
            ...
            <button
              ...
              className="font-display text-sm tracking-wider px-5 py-2 border border-blood/50 text-blood hover:bg-blood hover:text-white transition-all duration-300 glow-hover-red"
            >
              CONNEXION
            </button>
          )}
          ...
          >
            <div className="px-6 py-6 space-y-4">
              ...
              {isAuthenticated ? (
                ...
                <button
                  onClick={() => { loginWithRedirect(); setMenuOpen(false); }}
                  className="w-full font-display tracking-wider py-3 border border-blood text-blood hover:bg-blood hover:text-white transition-all"
                >
                  CONNEXION
                </button>
              )}
